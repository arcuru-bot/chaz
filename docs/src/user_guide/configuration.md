...
# Local frontends use the daemon's eidetica service socket instead of opening
# the embedded database. The default path is <state_dir>/eidetica.sock.
# service:
#   enabled: true
...
`chaz -p "<prompt>"` (alias `--print`) runs a single ReAct turn and exits.
It cannot approve tools interactively: `UnlessAutoApproved` tools follow the
daemon's ordinary `security.auto_approved_tools`, and `Always` tools fail closed.

`-p --session NAME` reuses a named session across invocations (find-or-create).
Without `--session` each invocation creates a fresh ephemeral session.
...
`eidetica.db` is an embedded SQLite backend, and an embedded backend belongs to one process: two processes that open it do not observe each other's writes and contend for the write lock.

Eidetica's answer is service mode — `chaz daemon` owns the backend and sync, serves it over a Unix socket, and every local frontend connects as a client:
...
1. Start any local frontend on a cold state directory. It starts the daemon and connects:
...
3. Stop the daemon with SIGTERM. A clean shutdown removes the socket and releases both advisory locks, so the next frontend starts it normally again. After a crash, the kernel still releases the locks; the next start replaces the stale socket file.
